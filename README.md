...
このリポジトリには、ローカルで差分を安全にレビューできる軽量ランナーを追加しました。目的は「ローカルで差分をスクラブ（機密マスク）して、LLM を使う/使わないにかかわらず自動レビューの出力を確認する」ことです。チームでの GitHub Actions を使わない個人開発ワークフロー向けの実装になっています。
